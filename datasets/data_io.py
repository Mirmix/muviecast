...
def scale_to_size(image: np.ndarray, size: Tuple[int, int]) -> np.ndarray:
    """Scale image to specified size

    Args:
        image: the input image in original size
        size: the size to scale the image down to
    ...
    return cv2.resize(image, size, interpolation=cv2.INTER_LINEAR), original_height, original_width
...
def read_image(filename: str, size: Tuple[int, int]) -> np.ndarray:
    ...
    np_image = np.array(image, dtype=np.float32) / 255.0
    if type(size) == int:
        return scale_to_max_dim(np_image, size)
    else:
        return scale_to_size(np_image, size)
