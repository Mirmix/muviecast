...
class CustomDataset(Dataset):
    def __init__(self, root_dir, split='', is_train=True, n_views=3, levels=3, img_wh=(1280, 704),scan_name ='Train',extension = 'png'):
        """
    ...
    def build_proj_mats(self):
        ...
        for scan in self.scans:
            ...
            vidlist = self.ref_views_per_scan[scan]
            if self.is_train == False:
                vidlist = [0]
            for vid in vidlist:
                proj_mat_filename = os.path.join(self.root_dir+"/"+self.scan_name,
    ...
    def define_transforms(self):
        ...
        self.untransform = T.Compose([T.Normalize(mean=[-0.485 / 0.229, -0.456 / 0.224, -0.406 / 0.225],
    ...
    def __getitem__(self, idx):
        ...
        for i, vid in enumerate(view_ids):
            img_filename = os.path.join(self.root_dir, self.split, scan, f'images/{vid:08d}'+self.extension)
            ...
            if self.is_train == False:
                proj_mat_ls, depth_min = self.proj_mats[scan][0]
            else:
